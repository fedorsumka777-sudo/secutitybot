...
# =========================================================
# SECURITY GUARD BOT v0.4
# Тестова версія БЕЗ PostgreSQL.
#
# ГОЛОВНЕ У v0.4:
# - повноцінний модуль "Навчання та перевірка знань"
# - 15 питань за одну спробу
# - усі 6 тематичних блоків присутні в кожному тесті
# - компактне меню без довгого прокручування
...
def main_menu(role):
    # Компактне меню: максимум 4 рядки для охоронника
    # і 5 рядків для старшого охоронника.
    rows = [
        [{"text": "🟢 Заступити"}, {"text": "🎓 Навчання"}],
        [{"text": "🚶 Обхід"}, {"text": "🔎 Об'єкт"}],
        [{"text": "⚠️ Недолік"}, {"text": "🚨 Подія"}],
        [{"text": "📋 Чергування"}, {"text": "📚 Інструкції"}],
    ]
...
def admin_menu():
    return {
        "keyboard": [
            [{"text": "👥 Заявки"}, {"text": "👮 Працівники"}],
            [{"text": "🎓 Результати"}, {"text": "📊 Стан системи"}],
        ],
        "resize_keyboard": True,
        "is_persistent": True
    }
...
def training_menu():
    return {
        "keyboard": [
            [{"text": "📝 Первинний тест"}, {"text": "🔄 Повторний тест"}],
            [{"text": "📚 Матеріали"}, {"text": "🧠 Мої результати"}],
            [{"text": "⬅️ Головне меню"}],
        ],
        "resize_keyboard": True,
        "is_persistent": True
...
def build_test():
    """
    Формує 15 питань і обов'язково охоплює всі 6 тем.
    Три випадково обрані теми дають по 3 питання,
    інші три теми — по 2 питання. Разом: 15.
    """
    selected = []
    topics = TOPICS[:]
...
def start_quiz(tg_id, mode="primary"):
    ...
    send_message(
        ...
        "У тесті <b>15 питань</b>. Усі 6 тематичних блоків представлені в кожній спробі.\n"
...
def handle_text(msg):
    ...
    if tg_id == ADMIN_ID:
        ...
        if text in ("👥 Заявки на доступ", "👥 Заявки"):
            data = load_data()
            rows = [
        ...
        if text in ("🎓 Навчання — результати", "🎓 Результати"):
            send_message(
                tg_id,
    ...
    if text == "🎓 Навчання":
        send_message(
            ...
            f"Первинна оцінка: {TEST_QUESTION_COUNT} питань.\n"
    ...
    if text in ("📝 Первинна оцінка знань", "📝 Первинний тест"):
        start_quiz(tg_id, mode="primary")
        return

    if text in ("🔄 Повторне тестування", "🔄 Повторний тест"):
        attempts = get_training_attempts(tg_id)
        if not attempts:
            send_message(
                tg_id,
    ...
    if text in ("📚 Навчальні матеріали", "📚 Матеріали"):
        send_message(
            tg_id,
...
if __name__ == "__main__":
    ...
    log.info(
        "Starting Security Guard Bot v0.4 on port %s",
        port
    )
    app.run(
        host="0.0.0.0",
